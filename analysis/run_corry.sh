...
# 遍历指定的区域，找出每个 region 的最小 VCASB 文件夹
for REGION in "${REGIONS[@]}"; do
    ...
    MIN_VCASB[$REGION]=$min_vcasb  # 将每个 region 的 VCASB 号保存到 MIN_VCASB 数组中

    # 生成目标文件名
    if [ "$HIGHSTAT" = true ]; then
        GEOMETRY_UPDATE_FILE="${GEOMETRY_UPDATE_DIR}/${TB_PERIOD}_3REF-MOSS-3REF_${MOSS}_${HFUnit}_region${REGION}_${min_vcasb}_highstat_masked.conf"
    else
