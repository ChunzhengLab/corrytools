...
void ClusteringSpatial::initialize() {
    ...
    title = m_detector->getName() + " Cluster Pixels Matrix;cluster pixels matrix;events";
    clusterPixelsMatrix = new TH1I("clusterPixelsMatrix", title.c_str(), 100, 0, 100);
}
...
StatusCode ClusteringSpatial::run(const std::shared_ptr<Clipboard>& clipboard) {
    ...
    for(auto pixel : pixels) {
        ...
        auto pixelMartix = cluster->pixelMatrix();
        // cluster->printPixelMatrix(std::cout);
        // std::cout<<getPixelMap(pixelMartix)<<std::endl;
        clusterPixelsMatrix->Fill(std::to_string(getPixelMap(pixelMartix)).c_str(), 1);
...
uint64_t ClusteringSpatial::getPixelMap(std::vector<std::vector<bool>>& pixelMatrix) {
    const size_t max_col_row = 8;
    if(pixelMatrix.empty()) {
        return 0;
    }
    // 检查矩阵的行或任何行的列是否超过8
    if (pixelMatrix.size() > max_col_row) {
        return 0;
    }
    for (const auto& row : pixelMatrix) {
        if (row.size() > max_col_row) {
            return 0;
        }
    }
    uint64_t pixelMap = 0;
    for (size_t i = 0; i < max_col_row && i < pixelMatrix.size(); ++i) {
        for (size_t j = 0; j < max_col_row && j < pixelMatrix[i].size(); ++j) {
            if (pixelMatrix[i][j]) {
                pixelMap |= (1ULL << (i * max_col_row + j));
            }
        }
    }
    return pixelMap;
